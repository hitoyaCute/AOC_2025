...
struct traveler {
    uint_fast8_t pos[2] = {0,0};

    bool operator==(const traveler& lhs) {
        const uint_fast16_t r = (pos[0] << 8) + (pos[1]);
        const uint_fast16_t l = (lhs.pos[0] << 8) + (lhs.pos[1]);
        return r == l;
    }
    struct Hasher {
        size_t operator()(traveler const& i) const noexcept {return std::hash<int>()((i.pos[0] << 8) + (i.pos[1]));}
    };
    /*
    ...
    */
    bool move(std::vector<std::string>& board, std::array<traveler, 2>* dumps) {
        int width, heigth;
        width = board[0].length();
        heigth = board.size();
        if (0 <= pos[0] and pos[0] <= width and 0 <= pos[1] and pos[1] <= heigth) {
